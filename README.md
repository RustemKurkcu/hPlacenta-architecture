...
- `could not find function "print_seurat_diagnostic"`

To avoid these issues, source the compatibility helpers before running downstream scripts:

If you see `could not find function "print_seurat_diagnostic"`, this is a utility-loading issue
(not a week-column issue). Make sure `scripts/R/utils.R` is sourced before the preprocess/mapping scripts.
...
### Quick pipeline diagnostic dump

If runs fail and you want a compact snapshot of scripts, objects, Seurat versions,
and key metadata (week/sample columns), run:

```bash
Rscript scripts/00_diagnose_pipeline_state.R
```

This writes:
- `output_share/logs/pipeline_diagnostic_<timestamp>.txt`
- `output_share/logs/pipeline_diagnostic_<timestamp>.json` (if `jsonlite` is installed)
